...
#let slides(doc) = {
	...
	show heading: title => {
		pagebreak(weak: true)
		rect(width: 100%, height: 100%, fill: theme_color, text(fill: white, title))
		pagebreak(weak: true)
	}
...
#let slide(title: "Chapter", content) = locate(loc => {
	...
	grid(rows: (auto, 1fr, auto), {
		...
		rect(width: 100%, fill: theme_color, pad(x: 10pt, y: 10pt)[
			#if headers == () {
		...
		// Inner slide content.
		pad(x: 10pt, y: 10pt, content)
	}, {
		// Footer with left and right section.
		grid(columns: (1fr, auto), pad(x: 5pt, y: 8pt)[
